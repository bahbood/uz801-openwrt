...
# ------------------------------------------------------------
# Fixed dependency commits (kept for reference / future use)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
echo
echo "[4/10] Installing UZ801 target files and packages..."

# Target
mkdir -p openwrt/target/linux/msm89xx
cp -a msm89xx/. openwrt/target/linux/msm89xx/

# Custom packages → package/custom (clean and standard)
rm -rf openwrt/package/custom
mkdir -p openwrt/package/custom
cp -a packages/. openwrt/package/custom/
...
if [ ! -d "awg-src" ]; then
    ...
    if [ "$CURRENT_AWG" != "$AWG_VERSION" ]; then
        echo
        echo "ERROR:"
        echo "Existing AWG source is: ${CURRENT_AWG:-unknown}"
        echo "Expected: $AWG_VERSION"
        exit 1
    fi
fi
...
cp -a awg-src/kmod-amneziawg       openwrt/package/
cp -a awg-src/amneziawg-tools      openwrt/package/
cp -a awg-src/luci-proto-amneziawg openwrt/package/

AWG_COMMIT="$(git -C awg-src rev-parse HEAD)"
echo
echo "AmneziaWG commit: $AWG_COMMIT"
...
echo
echo "Final target:"
grep '^CONFIG_TARGET_' .config | grep -E 'msm89xx|msm8916|yiming' || true

echo
echo "Important packages:"
grep '^CONFIG_PACKAGE_' .config | grep -E \
    'qmi|qrtr|rmtfs|wcn|modem|zhihe|sms|amnezia|firewall|dnsmasq|usb-gadget' || true
...
if [ "$BUILD_PACKAGES_ONLY" -eq 1 ]; then
    ...
    for pkg_dir in ../packages/*; do
        if [ -d "$pkg_dir" ] && [ -f "$pkg_dir/Makefile" ]; then
            ...
            echo "------------------------------------------------------------"
            make "package/custom/$pkg_name/compile" -j"$CORES" \
                || make "package/custom/$pkg_name/compile" -j1 V=s
        fi
    done
